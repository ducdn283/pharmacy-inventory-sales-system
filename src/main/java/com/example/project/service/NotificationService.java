...
public class NotificationService {
    ...
    @Transactional(readOnly = true)
    public List<NotificationResponse> latest(Integer accountId) {
        validateAccountId(accountId);

        return notificationRepository.findTop5ActiveForAccount(accountId)
                .stream()
                .map(NotificationResponse::from)
                .toList();
    }
    ...
    public long unreadCount(Integer accountId) {
        validateAccountId(accountId);
        return notificationRepository.countByAccountID_IdAndIsActiveTrueAndIsReadFalse(accountId);
    }

    @Transactional(readOnly = true)
    public NotificationStatsResponse getStats(Integer accountId) {
        validateAccountId(accountId);

        List<Notification> active = notificationRepository.findActiveForAccount(accountId);
        ...
        long urgent = active.stream()
                .filter(notification -> NotificationSeverity.URGENT.equals(notification.getSeverity()))
    ...
    @Transactional(readOnly = true)
    public Optional<NotificationResponse> findForAccount(Integer accountId, Integer notificationId) {
        validateAccountId(accountId);
    ...
    public void markAllAsRead(Integer accountId) {
        ...
        List<Notification> notifications = notificationRepository.findActiveForAccount(accountId);
    ...
    @Transactional
    public void dismiss(Integer accountId, Integer notificationId) {
        validateAccountId(accountId);
    ...
    @Transactional
    public Notification createIfMissing(Account receiver,
                                        String targetRole,
                                        String title,
                                        String message,
        ...
        if (receiver == null || receiver.getId() == null) {
            throw new IllegalArgumentException("Tài khoản nhận thông báo không hợp lệ");
        }

        if (dedupeKey != null && notificationRepository.existsByDedupeKeyAndIsActiveTrue(dedupeKey)) {
            return notificationRepository.findFirstByDedupeKeyAndIsActiveTrue(dedupeKey)
                    .orElse(null);
        }

        Notification notification = new Notification();
        notification.setAccountID(receiver);
        notification.setTitle(nonBlank(title, "Thông báo"));
        notification.setMessage(nonBlank(message, "Bạn có một thông báo mới"));
        notification.setNotificationType(nonBlank(notificationType, "SYSTEM"));
        notification.setCategory(nonBlank(category, NotificationCategory.HE_THONG));
        notification.setSeverity(nonBlank(severity, NotificationSeverity.INFO));
        notification.setStatus(NotificationStatus.UNREAD);
        notification.setTargetRole(nonBlank(targetRole, "SYSTEM"));
    ...
    public void resolveByDedupeKey(String dedupeKey) {
        ...
        notificationRepository.findFirstByDedupeKeyAndIsActiveTrue(dedupeKey)
    ...
    public void resolveByReference(String referenceType, Integer referenceId) {
        ...
        List<Notification> notifications =
                notificationRepository.findActiveByReference(referenceType, referenceId);
    ...
    private boolean isNotClosed(Notification notification) {
        return !NotificationStatus.RESOLVED.equals(notification.getStatus())
                && !NotificationStatus.DISMISSED.equals(notification.getStatus());
    }
