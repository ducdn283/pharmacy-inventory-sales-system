...
public class DashboardService {
    ...
    private static final String INVOICE_STATUS_CANCELLED =
            "Đã hủy";
    ...
    public DashboardService(
        ...
        this.stockadjustmentRepository =
                stockadjustmentRepository;
        this.shiftreportRepository =
                shiftreportRepository;
        this.approvalService = approvalService;
    }

    @Transactional(readOnly = true)
    public RoleDashboard ownerDashboard(
        ...
        List<ApprovalItemResponse> pendingApprovalItems =
                approvalService.list(null)
                        .stream()
                        .filter(
        ...
        return new RoleDashboard(
                ...
                "Tổng quan",
                currentAccountName,
                "Tổng quan vận hành nhà thuốc hôm nay"
                        + " • Ngày "
    ...
    public RoleDashboard pharmacistDashboard(
        ...
        List<Income> myEffectiveIncomes =
                effectiveIncomes.stream()
                        .filter(income ->
                                sameAccount(
                                        income.getApplicantID(),
                                        accountId
                                )
                        )
                        .toList();

        BigDecimal todayInvoiceSales =
                sumInvoiceTotal(
                        myInvoices,
                        invoice ->
                                isDate(
        ...
        BigDecimal todayInvoiceCash =
                sumInitialInvoicePayment(
                        myInvoices,
                        effectiveIncomes,
                        invoice ->
                                isDate(
        ...
        BigDecimal todayInvoiceBanking =
                sumInitialInvoicePayment(
                        myInvoices,
                        effectiveIncomes,
                        invoice ->
        ...
        BigDecimal todayCashCollected =
                todayInvoiceCash.add(
                        todayIncomeCash
                );

        BigDecimal todayBankingCollected =
                todayInvoiceBanking.add(
                        todayIncomeBanking
                );

        BigDecimal todayCollected =
                todayCashCollected.add(
        ...
        List<Return> todayApprovedReturns =
                returns.stream()
                        .filter(ret ->
                                sameAccount(
                                        ret.getReturnedBy(),
        ...
        BigDecimal todayRefundValue =
                todayApprovedReturns.stream()
        ...
        return new RoleDashboard(
                ...
                List.of(
                        ...
                        new MetricCard(
                                "Số lượng đơn hoàn",
                                String.valueOf(
                                        todayApprovedReturns.size()
                                ),
                                "Phiếu trả hàng đã được "
                                        + "duyệt hôm nay",
                                "ti ti-arrow-back-up",
                                "orange",
                                "/pharmacist/returns"
                        ...
                        new MetricCard(
                                "Giá trị tiền phải hoàn",
                                money(todayRefundValue),
                                "Tổng giá trị các đơn "
                                        + "hoàn hôm nay",
                                "ti ti-cash-off",
                                "danger",
                                "/pharmacist/returns"
        ...
        if (items.isEmpty()) {
            items.add(
                    new TodoItem(
                            ...
                            "Bạn chưa có phiếu nháp "
                                    + "hoặc công việc tồn đọng",
                            "Ổn định",
                            "success",
    ...
    private List<PerformanceRow> ownerPerformanceRows(
        ...
        Map<Integer, List<Invoice>> byEmployee =
                invoices.stream()
                        .filter(invoice ->
    ...
    private List<BigDecimal> hourlyRevenue(
        ...
        Map<Integer, BigDecimal> revenueByHour =
                new HashMap<>();
        ...
        invoices.stream()
                ...
                .forEach(invoice -> {
                    int hour =
                            invoice.getDate().getHour();

                    revenueByHour.merge(
                            hour,
                            safe(invoice.getTotal()),
                            BigDecimal::add
                    );
                });
    ...
    private String topSellingProductName(
        ...
        Map<String, Integer> quantityByProduct =
                new HashMap<>();
        ...
        return quantityByProduct.entrySet()
                .stream()
    ...
    private String compareRevenue(
        ...
        BigDecimal safeYesterday =
                safe(yesterday);

        if (safeYesterday.compareTo(
                BigDecimal.ZERO
        ) == 0) {
            return safeToday.compareTo(
                    BigDecimal.ZERO
            ) > 0
                    ? "+100%"
                    : "0%";
    ...
    private String formatInstant(
        ...
        return DATE_TIME_DISPLAY.format(
                instant.atZone(VN_ZONE)
        );
    }
    ...
    private String moneyShort(
        ...
        if (absoluteValue.compareTo(
            ...
            return million.stripTrailingZeros()
                    .toPlainString()
                    + "Mđ";
        }
